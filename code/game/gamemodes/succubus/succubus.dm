...
/datum/game_mode/succubus/post_setup()
	...
	while(LAZYLEN(possible_suckers))
...
/datum/game_mode/succubus/declare_completion()
	...
	LAZYREMOVE(succubi, SUC2)

	var/list/succers = list()
	LAZYADD(succers, SUC.current)
	LAZYADD(succers, SUC2.current)
