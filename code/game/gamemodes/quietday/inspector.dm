	...
	switch(href_list["acao"])
		if("joininspectree")
			...
			if(istype(src.mob, /mob/dead) || istype(src.mob, /mob/new_player))
				...
				if(!LAZYLEN(CandidatesForInspector)) // INSPECTOR SPAWN
					...
					to_chat(new_character, "<span class='dreamershitfuckcomicao1'>Objective #2:</span> <span class='dreamershit'>Leave a rating and abandon the fortress on the Ulysses.</span>")
...
/mob/living/carbon/human/proc/gradeHygiene()
	...
	set category = "gpc"
	var/hygiene = sanitize_num(input(usr, "What will be the grade - Choose between 0 and 100.", "ENOCH'S GATE HYGIENE") as num, 0, 100)
...
/mob/living/carbon/human/proc/gradePeople()
	...
	set category = "gpc"
	var/people = sanitize_num(input(usr, "What will be the grade? - Choose between 0 and 100.", "ENOCH'S GATE PEOPLE") as num, 0, 100)
	peopleGrade = people
...
/mob/living/carbon/human/proc/gradeFortress()
	...
	set category = "gpc"
	var/fortress = sanitize_num(input(usr, "What will be the grade? - Choose between 0 and 100.", "ENOCH's GATE BEAUTY") as num, 0, 100)
...
/mob/living/carbon/human/proc/RoundEnd()
	...
	while(higiene <= hygieneGrade)
		...
		if(higiene >= hygieneGrade)
			randAlpha = 255
		sleep(0.00001)
		T.Become("Enoch's Gate Hygiene: [higiene]", 20, -420, randAlpha, 200)
	while(fortaleza <= fortressGrade)
		...
		if(fortaleza >= fortressGrade)
			randAlpha = 255
		sleep(0.00001)
		T2.Become("Enoch's Gate Beauty: [fortaleza]", 20, -230, randAlpha, 200)
	while(pessoas <= peopleGrade)
		...
		if(pessoas >= peopleGrade)
			...
			world << "FOI"
		sleep(0.00001)
		T1.Become("Enoch's Gate: [pessoas]", 20, -48, randAlpha, 200)
