			...
			switch(href_list["usecrown"])
				if("declareemergency")
					if(!isEmergency)
						...
						to_chat(world, "<span class='excomm'>New [H.job]'s decree!</span>")
						world << sound('sound/AI/bell_toll.ogg')
						to_chat(world, "<span class='decree'>Our glorious [H.job] declares state of EMERGENCY!</span>")
						to_chat(world, "<br>")
						isEmergency = 1
						...
						to_chat(world, "<span class='excomm'>New [H.job]'s decree!</span>")
						world << sound('sound/AI/bell_toll.ogg')
						to_chat(world, "<span class='decree'>Our glorious [H.job] undeclares state of emergency!</span>")
						to_chat(world, "<br>")
						isEmergency = 0
						for(var/obj/machinery/emergency_room/E in emergency_rooms)
							if(E.activearea.alarm_toggled)
				...
				if("gathermeeting")
					if(!isMeeting)
						...
						to_chat(world, "<span class='excomm'>New [H.job]'s decree!</span>")
						world << sound('sound/AI/bell_toll.ogg')
						to_chat(world, "<span class='decree'>Our glorious [H.job] calls for a MEETING IN THE THRONE ROOM!</span>")
						to_chat(world, "<br>")
						isMeeting = 1
					else
						to_chat(world, "<span class='ravenheartfortress'>Enoch's Gate Hold</span>")
						to_chat(world, "<span class='excomm'>New [H.job]'s decree!</span>")
						world << sound('sound/AI/bell_toll.ogg')
						to_chat(world, "<span class='decree'>Our glorious [H.job] finishes the meeting!</span>")
				...
				if("appointhand")
					...
					if(fortHand)
						return
					for(var/mob/living/carbon/human/H in oview(1,usr))
						candidates += H
				...
				if("craftmedal")
					...
					var/input = sanitize(input(usr, "Choose someone to recieve the medal.", "Enoch's Gate Decree", "") as text|null)
					if(!input)
						return
