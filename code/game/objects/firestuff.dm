...
/obj/item/flame/torch/dropped(var/mob/user)
	...
	if(prob(rand(3, 7)))
		turn_off()
	if(!istype(src, /obj/item/flame/torch/lantern))
		if(prob(rand(3, 15)))
			for(var/atom/A in loc.contents)
				if(!A.flammable) continue
				new /obj/structure/fire(loc)

/obj/item/flame/torch/attackby(obj/item/W as obj, mob/living/carbon/human/user as mob)
	if(istype(W, /obj/item/flame))
		...
		if(F.loc == user.contents || loc == user.contents)
			user.on_fire = 1
			user.update_fire()
