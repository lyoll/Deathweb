...
*/
		...
		if(ticker.current_state == GAME_STATE_PLAYING)
			...
			if(rank == "Bum" || rank == "Migrant")
				...
				if(character.gender == MALE)
					...
					gender_prefix = "F"
				a.autosay("[character.real_name] ([rank ? " [rank];" : " visitor;"] [gender_prefix]/[character.age]) will arrive soon.", "Enoch's Gate CTTU")
	...
	proc/create_character(var/joined_late = 0)
		...
		if(client?.prefs.togglesize)
			new_character.penis_size = 30
			new_character.breast_size = "F"
